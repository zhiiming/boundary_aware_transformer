...
class DTOffsetLoader(data.Dataset):
    ...
    def __list_dirs(self, root_dir, dataset):
        ...
        if os.environ.get('use_cityscapes_style'):
            ...
            # root_dir = '/home/ilab/GitHub/openseg.pytorch/../dataset/dataset_ignore'
        ...
        label_dir = os.path.join(root_dir, 'SegmentationClassPNG')
        offset_dir = os.path.join(root_dir, 'offset_gt', offset_subdir)

        img_extension = os.listdir(image_dir)[0].split('.')[-1]

        # file_list_txt = os.environ.get('use_file_list')
        file_list_txt = 'ImageSets/Segmentation'
        if file_list_txt is None:
            Log.info_once('Using file list: all')
            files = sorted(os.listdir(label_dir))
        else:
            Log.info_once('Using file list: {}'.format(file_list_txt))
            with open(os.path.join(root_dir, file_list_txt, dataset + '.txt')) as f:
        ...
        for file_name in files:
            ...
            img_path = os.path.join(image_dir, '{}.{}'.format(file_name, img_extension))
            label_path = os.path.join(label_dir, '{}.{}'.format(file_name, 'png'))
            offset_path = os.path.join(offset_dir, '{}.{}'.format(file_name, 'mat'))
