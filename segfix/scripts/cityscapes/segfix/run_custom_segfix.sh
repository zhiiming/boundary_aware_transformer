...
# first parameter is task type: train / vat et al.
# second parameter is log number, for builing the output log dir.
export PYTHONPATH="$PWD":$PYTHONPATH

if [ -z $dt_max_distance ]; then
  export dt_max_distance=10
fi
...
PRETRAINED_MODEL="./pretrained_model/hrnetv2_w18_imagenet_pretrained.pth"


if [ "$1"x == "train"x ]; then
  ${PYTHON} -u main.py --configs ${CONFIGS} --drop_last y --test_interval ${TEST_INTERVAL} \
                       --phase train --gathered n --loss_balance y --log_to_file n \
                       --backbone ${BACKBONE} --model_name ${MODEL_NAME} --gpu 0 \
  ...
  ${PYTHON} -u main.py --configs ${CONFIGS} --drop_last y \
                       ...
                       --phase test_offset --gpu 0 --resume $CKPT \
